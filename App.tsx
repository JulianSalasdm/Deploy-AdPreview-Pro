...
const App: React.FC = () => {
  ...
  const processZip = async (file: File) => {
    ...
    try {
      ...
      setPreviewData({ type: AdType.ZIP, content: mainHtml, assets: finalAssets, fileName: file.name });
  ...
  const onDrop = useCallback((files: FileList) => {
    ...
    if (name.endsWith('.zip')) processZip(file);
      ...
      Papa.parse(file, {
        complete: (res) => {
          setPreviewData({ type: AdType.CSV, content: res.data[0]?.[0] || '', fileName: file.name });
          setActiveTab('preview');
        }
      });
      ...
      reader.onload = (e) => {
        ...
        setPreviewData({ type: AdType.TAG, content: rows[0]?.[0] || '', fileName: file.name });
        setActiveTab('preview');
      };
            ...
            {!previewData ? (
              <div className="space-y-6">
                <div className="bg-[#222] p-4 rounded-xl border border-[#333]">
                  <h3 className="text-xs font-semibold text-gray-500 uppercase mb-3 text-center">Configuración</h3>
